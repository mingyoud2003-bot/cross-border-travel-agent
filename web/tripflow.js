...
const CITY_TIMEZONES = {
  berlin:"Europe/Berlin", cologne:"Europe/Berlin", köln:"Europe/Berlin", frankfurt:"Europe/Berlin", munich:"Europe/Berlin",
  paris:"Europe/Paris", london:"Europe/London", vienna:"Europe/Vienna", zurich:"Europe/Zurich", rome:"Europe/Rome",
  madrid:"Europe/Madrid", barcelona:"Europe/Madrid", amsterdam:"Europe/Amsterdam", brussels:"Europe/Brussels",
...
function renderProviderCandidate(item, outcome) {
  return `<article class="provider-outcome candidate-card"><div class="provider-head"><h3>${escapeHtml(item.operator)} ${escapeHtml(item.flight_number)}</h3><span class="live-badge">${escapeHtml(item.flight_status)}</span></div><p>${escapeHtml(item.origin.name)} → ${escapeHtml(item.destination.name)}</p><p>${escapeHtml(zonedDisplay(item.departure_at,item.origin.timezone))} → ${escapeHtml(zonedDisplay(item.arrival_at,item.destination.timezone))}</p><small>联网时间 ${escapeHtml(item.checked_at)} · ${outcome.cached ? "缓存结果" : "实时请求"}</small><br><a class="attribution" href="${escapeHtml(outcome.attribution_url)}" target="_blank" rel="noreferrer">Flight data by AeroDataBox</a><button class="primary" data-confirm-kind="provider" data-candidate-id="${escapeHtml(item.candidate_id)}">确认并加入行程</button></article>`;
}

function renderTransportCandidate(item, index) {
  return `<article class="candidate-card"><h3>${item.mode === "flight" ? "航班" : "火车"} · ${escapeHtml(item.operator || "")}</h3><p>${escapeHtml(item.origin.name || item.origin.city)} → ${escapeHtml(item.destination.name || item.destination.city)}</p><p>${escapeHtml(item.departure_at)} → ${escapeHtml(item.arrival_at)}</p><button class="primary" data-confirm-kind="transport" data-index="${index}">确认并加入行程</button></article>`;
}
...
function renderItem(item) {
  ...
  return `<article class="trip-item"><div class="trip-time">${escapeHtml(zonedDisplay(item.departure_at,item.origin.timezone))}</div><div class="trip-main"><strong>${escapeHtml(item.origin.name)} → ${escapeHtml(item.destination.name)}</strong><span>${escapeHtml(item.operator)} ${escapeHtml(item.service_number || "")}</span></div><div class="trip-meta">${escapeHtml(zonedDisplay(item.arrival_at,item.destination.timezone,false))}<br><span class="badge">${provider ? "PROVIDER CHECKED · " : ""}USER CONFIRMED · R${item.revision}</span>${actions}</div></article>`;
}
