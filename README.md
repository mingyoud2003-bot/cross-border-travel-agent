...
v0.10.1 新增批量图片/PDF 导入：单个文件可提取多段航班、火车和住宿，
...
- 城市与机场/车站分层建模：中英文别名解析为服务端稳定 `city_id`，界面统一显示中文城市名；设施名仍保留供应商或用户原文。
- 济南/深圳、香港/洛杉矶等中英文城市别名映射到正确 IANA 时区；目录外城市不再显示伪默认值，必须由用户明确点选。
- 模型提出但用户原文没有出现的城市和站点会被删除。目录外名称可按用户原文保留，但不获得伪造的城市 ID，也不能跨语言自动判同。
...
当前确定性测试为 176/176。TripFlow Eval 数据集覆盖 100 个 case。文件导入另有
...
每次 push/PR 执行 176 项测试、两套 Eval 数据集静态校验、JavaScript/Python
语法检查和 Docker 构建；
